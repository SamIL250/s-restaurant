...
$stmt = $conn->prepare('INSERT INTO menu_items (item_name, category_id, price, cost, preparation_time, calories, is_vegetarian, is_vegan, is_gluten_free, is_available, image_url, description) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
$stmt->bind_param('siddiiiiiiss', $item_name, $category_id, $price, $cost, $preparation_time, $calories, $is_vegetarian, $is_vegan, $is_gluten_free, $is_available, $image_url, $description);
if (!$stmt->execute()) {
    $stmt->close();
